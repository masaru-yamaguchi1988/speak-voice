...
5. 話速、音高、抑揚、音量や、対応エンジン固有の感情・スタイルをスライダーで調整
6. テスト再生またはチャットを実行

文章入力欄ではEnterで改行し、Shift + Enterで送信します。「停止」を押すと、回答生成、再生中の音声、待機中の読み上げをまとめて中断できます。

「固定プロンプト・履歴設定」では、AIへの固定指示とプリセット、ブラウザ内への会話履歴保存、自動読み上げのON/OFFを設定できます。各メッセージからコピー、編集、再生成、再読み上げも行えます。会話はMarkdown形式で書き出せます。

VoiSona Talkを選択した場合は、画面に表示される接続設定へAPI URL、ユーザー名、API用パスワードを入力してください。話者を選ぶとALP、HUSと、そのボイスライブラリ固有のスタイルを調整できます。詳細は[VoiSona Talkの設定](docs/configuration.md#voisona-talk)を参照してください。
